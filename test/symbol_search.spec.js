...
describe('search algorithm', function () {
  let traded = isTraded("Rexnord of Indiana is moving to Mexico and rar viciously firing all of its 300 workers. This is happening all over our country. No more!");
  it('should find  correct stock symbol', function() {
...
describe('search algorithm', function () {
  let traded = isTraded("Don't believe  main stream (fake news) media. White House is running VERY WELL. I inherited a MESS and am in  process of fixing it.");
  it('should not find a ticker symbol', function() {
    expect(traded).to.deep.equal('');
  });
});
describe('search algorithm', function () {
  let traded = isTraded("One of  most effective press conferences I've ever seen! says Rush Limbaugh. Many agree.Yet FAKE MEDIA  calls it differently! Dishonest");
  it('should not find a ticker symbol', function() {
    expect(traded).to.deep.equal('');
  });
});
