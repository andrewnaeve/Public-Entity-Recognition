	...
	for (var k = 0; k < stockList.length; k++) {
		...
		for (var i = 0; i < phrase.length; i++) {
			...
			if (pat1.test(companyString) && phrase[i][0] === phrase[i][0].toUpperCase()) {
				...
				let matchObject = {
					match: t,
					symbol: stockList[k].symbol,
					description: stockList[k].description,
					relevance: relevance
				};
				...
				if (results[0].match.length < t.length && matchObject.match.length / companyString.length > minRelevance) {
					...
					if (
						results[0].match.split(" ").length / results[0].description.split(" ").length <
						matchObject.match.split(" ").length / matchObject.description.split(" ").length
					) {
						results[0] = matchObject;
					}
				}
